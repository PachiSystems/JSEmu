...
var MOS6502 = function() {
    ...
    this._statusRegister = {
        ...
        SIGN : false,
        dumpStatus : function() {
            return (this.CARRY ? 0x01 : 0x00) +
                (this.ZERO ? 0x02 : 0x00) +
                (this.INTERRUPT ? 0x04 : 0x00) +
                (this.DECIMAL ? 0x08 : 0x00) +
                (this.BREAK ? 0x10 : 0x00) +
                (this.UNUSED ? 0x20 : 0x00) +
                (this.OVERFLOW ? 0x40 : 0x00) +
                (this.SIGN ? 0x80 : 0x00);
        },
        restoreStatus : function (value) {
            this.CARRY = (value & 0x01) > 0;
            this.ZERO = (value & 0x02) > 0;
            this.INTERRUPT = (value & 0x04) > 0;
            this.DECIMAL = (value & 0x08) > 0;
            this.BREAK = (value & 0x10) > 0;
            this.UNUSED = (value & 0x20) > 0;
            this.OVERFLOW = (value & 0x40) > 0;
            this.SIGN = (value & 0x80) > 0;
        }
    };

    /* Stack */
    this._SP = 0xFF;  // For some reason the stack starts at the end and works backwards according to docs...
...
MOS6502.prototype.init = function() {
    ...
    me._statusRegister.CARRY = false;
    me._statusRegister.ZERO = false;
    me._statusRegister.INTERRUPT = false;
    me._statusRegister.DECIMAL = false;
    me._statusRegister.BREAK = true;
    me._statusRegister.UNUSED = true;
    me._statusRegister.OVERFLOW = false;
    me._statusRegister.SIGN = false;
...
MOS6502.prototype._PUSH = function(byte) {
    ...
    me._RAM[me._SP + 0x100] = (byte & 0xFF);
...
MOS6502.prototype.ReadZeroPageX = function(byte1) {
    return this._RAM[ ( byte1 + this._X ) & 0xFF];
};

MOS6502.prototype.ReadZeroPageY = function(byte1) {
    return this._RAM [ ( byte1 + this._Y ) & 0xFF];
};
...
/** HERE GOES THE COMMANDS THESE ARE IN THE ORDER THEY SHOULD BE IN THE TEST ROM **/
...
MOS6502.prototype.LDA = function() {
    ...
    me._statusRegister.SIGN = (OPER & 0x80) > 0;
    me._statusRegister.ZERO = OPER == 0;
    me._A = OPER;
...
MOS6502.prototype.CMP = function() {
    ...
    me._statusRegister.CARRY = me._A >= OPER;
...
MOS6502.prototype.BEQ = function() {
    ...
    if(me._statusRegister.ZERO) {

        // So... Since there are no signed numbers in JS... We have to work it out.
        var relAddress = ( OPER < 0x80 ) ? OPER + 2 : OPER - 256;
...
MOS6502.prototype.JMP = function() {
    ...
    switch (opCode) {
        ...
        case (0x4C):
            ...
            break;
        case (0x6C):
            var AddressStartByte = me._MAKE_ADDRESS(byte1,byte2);
            me._PC = me._MAKE_ADDRESS(me._RAM[AddressStartByte],me._RAM[AddressStartByte + 1]);
            me._CYCLES += 5;
            break;
...
MOS6502.prototype.LSR = function() {
    ...
    me._statusRegister.CARRY = (OPER & 0x01) > 0;
...
MOS6502.prototype.ASL = function() {
    ...
    //me._statusRegister.CARRY = (OPER >= 128);
    ...
    if(OPER != 0) {
        me._statusRegister.ZERO = false;
        me._statusRegister.CARRY = (tmp & 0x80) > 0;
    } else {
        me._statusRegister.ZERO = true;
    }
...
MOS6502.prototype.ROL = function() {
    ...
    var me = this,
        ...
        byte1 = me._RAM[ me._PC + 1],
        byte2 = me._RAM[ me._PC + 2],
        carry = me._statusRegister.CARRY ? 1 : 0,
        OPER;
...
MOS6502.prototype.ROR = function() {
    ...
    var me = this,
        ...
        byte1 = me._RAM[ me._PC + 1],
        byte2 = me._RAM[ me._PC + 2],
        carry = me._statusRegister.CARRY ? 0x80 : 0x00,
        OPER;
    ...
    me._statusRegister.SIGN = me._statusRegister.CARRY;

    me._statusRegister.CARRY = (OPER & 0x1) > 0;
...
MOS6502.prototype.DEX = function() {
    ...
    switch (opCode) {
        ...
        case (0xCA):
            me._X = (me._X - 1) & 0xFF;
            me._statusRegister.SIGN = (me._X & 0x80) > 0;
            me._statusRegister.ZERO = me._X == 0;
            me._CYCLES += 2;
            me._PC += 1;
            break;
...
MOS6502.prototype.TXA = function() {
    ...
    switch (opCode) {
        ...
        case (0x8A):
            me._A = me._X;
            me._statusRegister.SIGN = (me._A & 0x80) > 0;
            me._statusRegister.ZERO = me._A == 0;
            me._CYCLES += 2;
            me._PC += 1;
            break;
...
MOS6502.prototype.TAY = function() {
    ...
    switch (opCode) {
        ...
        case (0xA8):
            me._Y = me._A;
            me._statusRegister.SIGN = (me._Y & 0x80) > 0;
            me._statusRegister.ZERO = me._Y == 0;
            me._CYCLES += 2;
            me._PC += 1;
            break;
...
MOS6502.prototype.ADC = function() {
    ...
    var tmp = (me._A & 0xF) + (OPER & 0xF) + (me._statusRegister.CARRY ? 1 : 0);

    me._statusRegister.OVERFLOW = !(((me._A ^ OPER) & 0x80) != 0) && (((me._A ^ tmp) & 0x80) != 0);

    if (me._statusRegister.DECIMAL) {

        tmp = (me._A & 0xF) + (OPER & 0xF) + (me._statusRegister.CARRY ? 1 : 0);
        ...
        if (tmp > 0x99) {
            ...
            me._statusRegister.CARRY = true;
            if (me._statusRegister.OVERFLOW && tmp >= 0x180) { me._statusRegister.OVERFLOW = false; }
            ...
            me._statusRegister.CARRY = false;
            if (me._statusRegister.OVERFLOW && tmp < 0x80) { me._statusRegister.OVERFLOW = false; }

        }

    } else {

        tmp = me._A + OPER + (me._statusRegister.CARRY ? 1 : 0);

        if (tmp > 0xFF) {

            me._statusRegister.CARRY = true;
            me._statusRegister.OVERFLOW = true;
...
MOS6502.prototype.SBC = function() {
    ...
    if (me._statusRegister.DECIMAL) {
        ...
        tmp = 0xF + (me._A & 0xF) - (OPER & 0xF) + (me._statusRegister.CARRY ? 1 : 0);
        ...
        if (w < 0x100) {
            ...
            if(me._statusRegister.OVERFLOW && w < 0x80) { me._statusRegister.OVERFLOW = false; }
            ...
            if(me._statusRegister.OVERFLOW && w >= 0x180) { me._statusRegister.OVERFLOW = false; }
        ...
        w = 0xFF + me._A - OPER + (me._statusRegister.CARRY ? 1 : 0);
...
MOS6502.prototype.CPX = function() {
    ...
    me._statusRegister.CARRY = (me._X > OPER);
...
MOS6502.prototype.CPY = function() {
    ...
    me._statusRegister.CARRY = me._Y > OPER;
...
MOS6502.prototype.BIT = function() {
    ...
    me._statusRegister.SIGN = (OPER & 0x80) > 0;
    me._statusRegister.OVERFLOW = (OPER & 0x40) > 0; // Copy bit 6 to overflow flag.
    me._statusRegister.ZERO = (me._A & OPER) == 0;
...
MOS6502.prototype.BVC = function() {
    ...
    if(!me._statusRegister.OVERFLOW) {
...
MOS6502.prototype.BCS = function() {
    ...
    if (me._statusRegister.CARRY) {
        ...
        // So... Since there are no signed numbers in JS... We have to work it out.
        var relAddress = (OPER < 0x80 ) ? OPER + 2 : OPER + 2 - 256;
...
MOS6502.prototype.CLC = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0x18): me._CYCLES += 2; me._PC += 1; me._statusRegister.CARRY = false; break;
...
MOS6502.prototype.CLV = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0xB8): me._CYCLES += 2; me._PC += 1; me._statusRegister.OVERFLOW = false; break;
...
MOS6502.prototype.PHP = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0x08):
            me._PUSH(me._statusRegister.dumpStatus());
            me._CYCLES += 3;
            me._PC += 1;
            break;
...
MOS6502.prototype.PLP = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0x28):
            me._statusRegister.restoreStatus(me._PULL());
            me._CYCLES += 4;
            me._PC += 1;
            break;
...
MOS6502.prototype.SEI = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0x78):
            me._statusRegister.INTERRUPT = true;
            me._CYCLES += 2;
            me._PC += 1;
            break;
...
MOS6502.prototype.SED = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0xF8):
            me._statusRegister.DECIMAL = true;
            me._CYCLES += 2;
            me._PC += 1;
            break;
...
MOS6502.prototype.CLI = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0x58): me._CYCLES += 2; me._PC += 1; me._statusRegister.INTERRUPT = false; break;
...
MOS6502.prototype.CLD = function() {
    ...
    switch (opCode) {
        // Get Operand
        case (0xD8): me._CYCLES += 2; me._PC += 1; me._statusRegister.DECIMAL = false; break;
